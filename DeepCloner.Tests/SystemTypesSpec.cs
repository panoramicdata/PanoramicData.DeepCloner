...
public class SystemTypesSpec() : BaseTest(true)
{
	[Fact]
	public void StandardTypes_Should_Be_Cloned()
	{
		...
		Assert.Equal("test1", cloned.ToString());
		var arr = new[] { 1, 2, 3 };
		var enumerator = arr.GetEnumerator();
		enumerator.MoveNext();
		var enumCloned = enumerator.DeepClone();
		enumerator.MoveNext();
		Assert.Equal(1, enumCloned.Current);
	}
	...
	public void Funcs_Should_Be_Cloned()
	{
		...
		closure[0] = "xxx";
		Assert.Equal("xxx3", f(3));
		// we clone delegate together with a closure
		Assert.Equal("1233", df(3));
		Assert.Equal("xxx3", cf(3));
	}
	...
	// Some libraries notifies about problems with this types
	[Fact]
	public void Events_Should_Be_Cloned()
	{
		var eht = new EventHandlerTest1();
		var summ = new int[1];
		void a1(int x) => summ[0] += x;
		void a2(int x) => summ[0] += x;
		eht.Event += a1;
		eht.Event += a2;
		eht.Call(1);
		Assert.Equal(2, summ[0]);
		...
		eht.Event -= a1;
		eht.Event -= a2;
		Assert.Equal(0, eht.Call(1)); // 0
		Assert.Equal(2, summ[0]); // nothing to increment
		Assert.Equal(2, clone.Call(1));
	}
	...
	{
       Assert.Skip("X509Certificate2 disposal/finalization currently crashes the .NET 10 test host for this fixture.");
