<#
.SYNOPSIS
    Publishes the PanoramicData.DeepCloner package to NuGet.org
...
if (-not $SkipTests) {
    ...
    dotnet test "$PSScriptRoot\DeepCloner.Tests\PanoramicData.DeepCloner.Test.csproj" --configuration Release --no-restore
    if ($LASTEXITCODE -ne 0) {
...
Write-Information "Building package..." -InformationAction Continue
    dotnet build "$PSScriptRoot\DeepCloner\PanoramicData.DeepCloner.csproj" --configuration Release
if ($LASTEXITCODE -ne 0) {
    Write-Error "ERROR: Build failed."
    exit 1
}

$packagePath = Join-Path $PSScriptRoot "DeepCloner\bin\Release\PanoramicData.DeepCloner.$version.nupkg"
...
Write-Information "Successfully published PanoramicData.DeepCloner version $version to nuget.org!" -InformationAction Continue
exit 0
